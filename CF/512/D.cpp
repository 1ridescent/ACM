...
void input()
{
	...
	for(int i = 1; i <= N; i++) if(!G[i].empty()) force_root[i] = true;
	for(int i = 1; i <= N; i++)
		for(int j = 1; j <= N; j++)
...
int main()
{
	...
	for(int r = 1; r <= N; r++)
	{
		...
		for(int i = 1; i <= N; i++)
			...
			{
				root[i] = r;
				if(force_root[i]) forced = i;
			}
		if(forced)
		{
			//cout << forced << " forced\n";
			compute(forced);
			memcpy(temp, dp[forced], sizeof(temp));
		}
		else
		{
			for(int u = 1; u <= N; u++)
		...
		//cout << r << " : "; for(int k = 0; k <= 10; k++) cout << temp[k] << ' '; cout << endl;
